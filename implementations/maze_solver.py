...
def canMoveToCallback(mt: MatrixTraverser, 
    ...
    if currCoord.isStart:
        return Matrix.getAtCoordinate(mt.matrix, desiredCoord) == getFirstNextValueAtStart() 
    ...
    if Matrix.getAtCoordinate(mt.matrix, desiredCoord) == getStartName():
        return False
    
    # if the next move is the end, you can move
    if Matrix.getAtCoordinate(mt.matrix, desiredCoord) == getEndName():
        return True
    
    # if the curr coordinate is the end itself,
    # end the algorithm
    if Matrix.getAtCoordinate(mt.matrix, currCoord) == getEndName():
        # end the algorithm at 
        StateManager.setWasEnded(mt, True)
        return False

    # user-defined "can move" at the cell level
    return canMove(mt, desiredCoord, prevCoord, currCoord, prevMove)
